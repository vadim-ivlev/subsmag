<?php
/**
 * Copied and a bit refactored from works.rg.ru, MoneyToText
 */
...
class PriceToTextConverter
{
    ...
    public function convert(int $int, int $dec)
    {
        ...
        if($rubli)
        {
            ...
            if($rubli >= 1000000000)
                $ret .= $this->textHundreds($rubli, 1000000000,
                        array('миллиард', 'миллиарда', 'миллиардов')) . ' ';
            if($rubli >= 1000000)
                $ret .= $this->textHundreds($rubli, 1000000,
                        array('миллион', 'миллиона', 'миллионов') ) . ' ';
            if($rubli >= 1000)
                $ret .= $this->textHundreds($rubli, 1000,
                        array('тысяча', 'тысячи', 'тысяч'), true) . ' ';

            $ret .= $this->textHundreds($rubli, 1, self::RUB) . ' ';

            $ret = mb_strtoupper(mb_substr($ret, 0, 1)).mb_substr($ret, 1, mb_strlen($ret));
        }
        if($kopeiki < 10)
            $ret .= '0';
        $ret .= $kopeiki . ' ' . self::KOP[ $this->descrIdx($kopeiki) ];
    ...
    private function textHundreds(&$in, $raz, $ar_descr, $fem = false)
    {
        $ret = '';
